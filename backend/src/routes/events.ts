...
async function findOrCreateCustomer(tenantId: number, fullName: string | null) {
...
async function insertTimelineEvent(params: {
  ...
  await pool.query(
    ...
    [tenant_id, job_id, kind, message, JSON.stringify(meta || {})]
  );
}

function normalizeStage(toValue: any): string {
  const to = (toValue ?? "").toString().trim();

  // Keep this explicit for now (you can expand later)
  if (to === "Estimate Sent") return "estimate_sent";
  if (to === "Lead") return "lead";
