...
async function registerRouteModule(app, modulePath, preferredExport) {
    try {
...
function startAutoScheduler(app) {
    ...
    if (!enabled) {
        app.log.warn({ enabled, intervalMs, limit }, "Auto-scheduler disabled (set SCHEDULER_ENABLED=true to enable)");
        return { stop: () => { } };
    }
    let running = false;
    let timer = null;
