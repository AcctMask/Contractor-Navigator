...
function normalizeStage(stageRaw: string): CanonStage {
  ...
  if (s.includes("work auth")) return "work_auth_sent";
  if (s.includes("estimate")) return "estimate_sent";
  if (s.includes("contract")) return "contract_sent";
  if (s.includes("final") || s.includes("paid") || s.includes("complete")) return "final_paid";

  return "unknown";
}

function sequenceForStage(stage: CanonStage): string | null {
  switch (stage) {
    case "lead":
...
async function upsertJobFromStageChange(tenantId: number, payload: any, canonStage: CanonStage) {
    ...
    await pool.query(
      `update jobs
       set stage=$1,
           ...
           updated_at=now()
       where id=$6`,
      [canonStage, address1, city, state, zip, id]
    );
  ...
  const inserted = await pool.query(
    `insert into jobs (tenant_id, external_crm, external_job_id, stage, address1, city, state, zip)
     values ($1, 'jobprogress', $2, $3, nullif($4,''), nullif($5,''), nullif($6,''), nullif($7,''))
     returning id`,
    [tenantId, jobId, canonStage, address1, city, state, zip]
  );
...
async function startOrUpdateAutomation(tenantId: number, jobDbId: number, sequenceKey: string) {
  // If an active automation exists for this job+sequence, keep it; else create fresh active state.
  const existing = await pool.query(
    `select id, status, step_index from automation_state
     where tenant_id=$1 and job_id=$2 and sequence_key=$3
     order by id desc
     limit 1`,
    [tenantId, jobDbId, sequenceKey]
  );
  ...
  await pool.query(
    `insert into automation_state (tenant_id, job_id, sequence_key, status, step_index, next_run_at)
...
export async function runDecisionMatrix(e: EventInput) {
  // Always log receipt
